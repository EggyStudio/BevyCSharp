...
pub fn html_fn(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let (event_variant, event_type) = match extract_event_type(&input_fn) {
        ...
        Ok(None) => (
            ...
            syn::parse_quote!(bcs_ui::html::HtmlEvent),
        ),
        Err(err) => {
    ...
    let expanded = quote! {
        ...
        bcs_ui::html::inventory::submit! {
            bcs_ui::html::HtmlFnRegistration::#event_variant {
                name: #name,
                build: #builder_ident,
            }
        }
...
pub fn component_init(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        bcs_ui::html::inventory::submit! {
            bcs_ui::html::ComponentInitRegistration {
...
fn expand_resource_shared_registrations(
    ...
    for arg in &input_fn.sig.inputs {
        ...
        registrations.push(quote! {
            ...
            ) -> Option<bcs_ui::lang::serde_json::Value> {
                world
                    .get_resource::<#resource_ty>()
                    .and_then(|value| bcs_ui::lang::serde_json::to_value(value).ok())
            }

            bcs_ui::lang::inventory::submit! {
                bcs_ui::lang::HtmlSharedRegistration::Shared {
                    name: #name_lit,
...
/// `bcs_ui` by scanning component definition files.
#[proc_macro_attribute]
...
pub fn beu_routes(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        bcs_ui::routing::inventory::submit! {
            bcs_ui::routing::RoutesRegistration {
...
fn expand_beu_store(input: DeriveInput) -> Result<proc_macro2::TokenStream> {
    ...
    Ok(quote! {
        impl bcs_ui::framework::BeuStore for #ident {
        ...
        #[doc(hidden)]
        #[allow(non_snake_case)]
        fn #register_ident(store: &mut bcs_ui::framework::UiBindingStore) {
        ...
        bcs_ui::framework::inventory::submit! {
            bcs_ui::framework::UiBindingStoreRegistration {
                key: stringify!(#ident),
