...
/// What a program has decided about an element's box, which the stylesheet does not undo.
///
/// A stylesheet describes what a kind of thing looks like. Where a particular one goes, how large
/// it is and whether it is on screen at all are frequently decided while the program runs: a panel
/// dragged to a corner, a row that is scrolled off, a list that is as tall as what is in it. Both
/// of those end up in the same `Node`, and applying the stylesheet writes the whole of it, so
/// without somewhere to say "this part is mine" a program's decisions last until the next restyle
/// and no longer, which is a panel that jumps back to the corner of the screen when anything in it
/// is touched.
///
/// Every field is optional and nothing here is a default: what is set overrides the stylesheet,
/// and what is not is left to it. Take a field back by setting it to `None`, not by writing what
/// the stylesheet used to say.
#[derive(Component, Reflect, Debug, Clone, Default, PartialEq)]
#[reflect(Component)]
pub struct StyleOverride {
    /// Whether the element is laid out at all.
    pub display: Option<Display>,

    /// How the element is placed: in the flow, or against its parent's box.
...
impl StyleOverride {
    /// Whether anything has been decided at all.
    ...
    pub fn apply(&self, node: &mut Node) {
        ...
        if let Some(position) = self.position_type {
            node.position_type = position;
        }
...
impl FontWeight {
    ...
    /// Returns the numeric weight (100 to 900)
