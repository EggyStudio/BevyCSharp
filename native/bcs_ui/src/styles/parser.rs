...
pub fn apply_property_to_style(style: &mut Style, name: &str, value: &str) {
    match name {
        ...
        "align-content" => style.align_content = convert_to_bevy_align_content(value.to_string()),
        "align-self" => style.align_self = convert_to_bevy_align_self(value.to_string()),
        "flex-direction" => {
...
fn convert_to_font_weight(value: String) -> Option<FontWeight> {
    ...
    // 1) Try numeric first (CSS-style: 1 to 1000, we clamp logically)
...
///
/// Accepts 1 to 4 values (e.g. `"10px"`, `"10px 20px"`, etc.), similar to CSS shorthand.
...
/// Converts a string into an `AlignContent` value.
///
/// What a box with more than one line of children does with the room left over. Without it a
/// wrapping box taller than its lines spreads them down its height with no way to say otherwise,
/// which is a grid of tiles with its rows pushed apart.
pub fn convert_to_bevy_align_content(value: String) -> Option<AlignContent> {
    match value.trim() {
        ...
        _ => Some(AlignContent::default()),
    }
}

/// Converts a string into an `AlignSelf` value, which is one child disagreeing with its parent.
pub fn convert_to_bevy_align_self(value: String) -> Option<AlignSelf> {
    match value.trim() {
