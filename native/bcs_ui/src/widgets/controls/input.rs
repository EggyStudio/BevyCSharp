    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            ...
            (
                ...
                handle_typing,
                apply_pending_paste,
                refresh_text_when_node_appears,
...
/// Says an input changed when the node that draws its text appears.
///
/// What a widget shows lives on a node spawned a frame or two after the widget itself, and what
/// fills that node runs when the field changes. So a value written before the node existed is
/// held by the field, noticed with nothing to write it to, and drawn by nobody: the field says one
/// thing and the screen says another until the next time somebody happens to change it. Saying the
/// field changed when the node turns up is the whole fix.
fn refresh_text_when_node_appears(
    appeared: Query<&BindToID, Added<InputFieldText>>,
    mut fields: Query<(&mut InputField, &UIGenID), With<InputFieldBase>>,
) {
    if appeared.is_empty() {
        return;
    }

    for bind in appeared.iter() {
        for (mut field, id) in fields.iter_mut() {
            if id.0 != bind.0 {
                continue;
            }
