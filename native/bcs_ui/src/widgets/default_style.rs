...
/*
 * A document's body is the window it is drawn in.
 *
 * Sized by its contents it is nothing at all, which is fine for a panel placed by its left and top
 * edges and wrong for everything else: an element anchored to the right or the bottom is anchored
 * to the right or bottom of nothing, and lands off the screen. A body that fills the window is
 * what every document is written against.
 *
 * It does not take the pointer. Every open document lays one of these over the whole window, so a
 * body that answered a click would swallow every click meant for the document under it, and for
 * whatever the program is drawing behind them all. What wants clicks says so.
 */
body {
    position: absolute;
    left: 0;
