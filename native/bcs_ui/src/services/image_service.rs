...
pub fn get_or_load_image(
    ...
    if let Some(embedded_png) = embedded_icon_bytes(path) {
        if !asset_exists_in_project(path) {
            // The ordinary case rather than a fault: a program that ships no pictures of its
            // own gets the ones compiled in, which is why they are compiled in.
            debug!("no '{}' under the asset root, drawing the one compiled in", path);
...
pub fn pre_load_assets(
    ...
    // Nothing is wrong when it is not there. The folder is where a game puts pictures it wants
    // loaded before anything asks for them, and a game that has none is the ordinary case: the
    // widgets that need a picture of their own carry theirs compiled in.
    if !folder.exists() {
        debug!("no folder of images to preload at '{}'", folder.display());
        return;
    }
