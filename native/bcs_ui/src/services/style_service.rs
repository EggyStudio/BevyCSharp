...
use crate::styles::StyleOverride;
use crate::styles::{
...
type UiStyleComponents<'w, 's> = (
    ...
    // Last on purpose: the ones before it are addressed by position elsewhere.
    Option<Mut<'w, StyleOverride>>,
);
...
fn mark_new_nodes_for_style_refresh(
    ...
    for entity in query.iter() {
        // Quietly, because the node may be gone by the time the command runs. A widget that is
        // spawned and despawned inside one frame is an ordinary thing for a panel driving a pool
        // of rows to produce, and marking a dead entity for a restyle is not a fault: it is a
        // restyle that no longer needs doing.
        commands.entity(entity).queue_silenced(
            move |mut entity: EntityWorldMut| {
                entity.insert(StyleRefreshOnNodeAdded);
            },
        );
    }
}
...
fn apply_style_components(
    ...
    // Node, and then whatever the program has decided about the same fields, which the stylesheet
    // does not get to undo.
    let over = components.13.as_ref().map(|over| (*over).clone());
...
fn apply_style_to_node(style: &Style, node: Option<&mut Node>) {
    if let Some(node) = node {
        ...
        node.align_content = style.align_content.unwrap_or_default();
        node.align_self = style.align_self.unwrap_or_default();
