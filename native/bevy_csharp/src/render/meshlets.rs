    ...
    working.retain_mut(|conversion| {
        let Some(task) = &mut conversion.task else {
            ...
            conversion.task = Some(bevy::tasks::AsyncComputeTaskPool::get().spawn(async move {
                MeshletMesh::from_mesh(&mesh?, quantization).map_err(|error| error.to_string())
            }));
...
/// and zero takes Bevy's default of four, a sixteenth of a centimeter.
///
/// Returns [`status::UNSUPPORTED`] where meshlets are not running, and [`status::NO_COMPONENT`]
/// where the key names no mesh.
#[unsafe(no_mangle)]
pub extern "C" fn bcs_render_create_meshlet_mesh(mesh: i32, quantization: u32) -> i32 {
    crate::interop::guard(|| {
        #[cfg(not(feature = "meshlet"))]
        {
            let _ = (mesh, quantization);
            status::UNSUPPORTED
        }
