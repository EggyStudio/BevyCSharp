...
/// <remarks>
/// <para>
/// A table-stored component sits in a contiguous column, which makes iteration fast, since a query
/// walks it with no indirection. The cost is paid on insertion and removal, because both move the
/// entity to a different archetype and copy every one of its other components along with it.
/// </para>
/// <para>
...
/// storage behind a sparse set, only a per-entity lookup, so there is nothing for a chunk to point
/// at. <see cref="EcsWorld.Query{T}"/> and
/// <see cref="EcsWorld.Chunks{T}(ReadOnlySpan{int}, ReadOnlySpan{int}, bool)"/> refuse it.
/// Everything else works, meaning <see cref="EcsWorld.Add{T}"/>, <see cref="EcsWorld.Remove{T}"/>,
/// <see cref="EcsWorld.Has{T}"/>, <see cref="EcsWorld.GetRef{T}"/>,
/// <see cref="EcsWorld.Count{T}"/>, and naming it in a <c>[With]</c> or <c>[Without]</c> filter on
/// some other query, which is its purpose.
/// </para>
/// <para>
