...
/// behaviour and its components.
/// </para>
/// <para>
/// <b>Threading.</b> These methods are only valid on the main thread while a system is
/// running, because that is when Bevy loans its world out. A behaviour method running on a
...
public sealed unsafe class EcsWorld
{
    ...
    public void Add<T>(Entity entity, T component) where T : unmanaged =>
        Native.Check(
            Native.bcs_ecs_insert(entity.Bits, ComponentType<T>.Id, &component),
            $"Add<{typeof(T).Name}>");
    ...
    {
        var pointer = Native.bcs_ecs_get_ptr(entity.Bits, ComponentType<T>.Id);
        if (pointer is null)
        {
            component = default;
            return false;
    ...
    /// across a spawn, despawn, insert or remove.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// This is the primitive the generated behaviour runners use. Each chunk is a direct view
    ...
    {
        using var chunks = Chunks<T>(markChanged: false);
